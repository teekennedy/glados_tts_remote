...
class ModelConfig:
    ...
    def __init__(
        ...
        self.MODEL_PATH = (
            ...
            else resource_path("models/TTS/phomenizer_en.onnx")
        )
        self.PHONEME_DICT_PATH = (
            phoneme_dict_path
            if phoneme_dict_path is not None
            else resource_path("models/TTS/lang_phoneme_dict.pkl")
        )
        self.TOKEN_TO_IDX_PATH = (
            token_to_idx_path
            if token_to_idx_path is not None
            else resource_path("models/TTS/token_to_idx.pkl")
        )
        self.IDX_TO_TOKEN_PATH = (
            idx_to_token_path
            if idx_to_token_path is not None
            else resource_path("models/TTS/idx_to_token.pkl")
        )
...
class Phonemizer:
    ...
    def __init__(self, config: ModelConfig | None = None) -> None:
        ...
        if config is None:
            config = ModelConfig()
        self.config = config
        self.phoneme_dict: dict[str, str] = self._load_pickle(
            self.config.PHONEME_DICT_PATH
        )

        self.phoneme_dict["glados"] = (
            "ɡlˈɑːdɑːs"  # Add GLaDOS to the phoneme dictionary!
        )
    ...
    @staticmethod
    def _remove_padding(
        arr: list[NDArray[np.int64]], padding_value: int = 0
    ...
    @staticmethod
    def _trim_to_stop(
        arr: list[NDArray[np.int64]], end_index: int = 2
    ) -> list[NDArray[np.int64]]:
        """
    ...
    def _process_model_output(
        self, arr: list[NDArray[np.int64]]
    ) -> list[NDArray[np.int64]]:
        """
