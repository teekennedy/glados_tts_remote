...
def read_metadata(
    path: Path, metafile: str, format: str, n_workers=1
) -> Tuple[dict, dict]:
    if format == "ljspeech":
        ...
        raise ValueError(
            f'Metafile has unexpected ending: {path.stem}, expected [.csv, .tsv]"'
        )
...
def read_ljspeech_format(path: Path, multispeaker: bool = False) -> Tuple[dict, dict]:
    ...
    with open(str(path), encoding="utf-8") as f:
        for line in f:
            split = line.split("|")
            speaker_name = (
                split[-2] if multispeaker and len(split) > 2 else DEFAULT_SPEAKER_NAME
            )
...
def read_vctk_format(path: Path, n_workers: int, extension=".txt") -> Tuple[dict, dict]:
    files = get_files(path, extension=extension)
    text_dict = {}
    speaker_dict = {}
    pool = Pool(processes=n_workers)
    for i, (file, text) in tqdm.tqdm(
        enumerate(pool.imap_unordered(read_line, files), 1), total=len(files)
    ):
